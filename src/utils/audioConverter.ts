...
export const isAudioFile = async (filePath: string): Promise<boolean> => {
  try {
    ...
    const { stdout } = await execAsync(`"${ffmpegPath}" -i "${filePath}" -f null - 2>&1`);
    return !stdout.includes('Invalid data found when processing input');
  } catch (error: any) {
    // ffmpegがエラーを返すが、音声ファイルであれば特定のエラーメッセージを含まない
    if (error.stderr && !error.stderr.includes('Invalid data found when processing input')) {
      return true;
    }
    return false;
  }
...
export const convertToWav = async (filePath: string): Promise<string> => {
  ...
  const outputPath = path.join(tempDir, `${path.basename(filePath, ext)}_converted.wav`);
  
  try {
    const ffmpegPath = getFfmpegPath();
    await execAsync(`"${ffmpegPath}" -i "${filePath}" -acodec pcm_s16le -ar 44100 "${outputPath}"`);
    return outputPath;
  } catch (error: any) {
