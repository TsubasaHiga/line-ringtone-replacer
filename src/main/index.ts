...
const createWindow = () => {
  ...
  if (isDev) {
    ...
    for (const htmlPath of htmlPaths) {
      console.log(
        `index.htmlの検索: ${htmlPath} - ${
          fs.existsSync(htmlPath) ? "存在します" : "存在しません"
        }`
      );
    ...
    if (indexHtmlPath) {
      ...
      mainWindow.loadURL(
        `data:text/html,<html><body><h1>エラー</h1><p>index.htmlファイルが見つかりませんでした。</p></body></html>`
      );
    }
  }
...
app.whenReady().then(() => {
  ...
  const getPreloadPath = () => {
    ...
    console.error(
      "プリロードスクリプトが見つかりませんでした。デフォルトパスを使用します。"
    );
      ...
      if (fs.existsSync(dir.path)) {
        ...
        if (fs.existsSync(distPath)) {
          ...
          try {
            ...
            console.log(
              `index.html存在チェック: ${
                fs.existsSync(indexPath) ? "存在します" : "存在しません"
              }`
            );
          } catch (err) {
...
ipcMain.handle("restore-ringtone", async () => {
  try {
    ...
    if (!lineDataPath) {
      return {
        success: false,
        message: "LINEのデータディレクトリが見つかりません。",
      };
    }
    ...
    const isUsingTestDir =
      isDev &&
      useTestDirInDev &&
      lineDataPath.includes("line-ringtone-replace-test");
      ...
      return new Promise((resolve) => {
        try {
          ...
          resolve({
            success: true,
            message:
              "デフォルトの着信音に戻しました。バックアップファイルも削除しました。",
          });
        } catch (e: unknown) {
          ...
          resolve({
            success: false,
            message: `ファイル操作に失敗しました: ${errorMessage}`,
          });
        }
      });
    }
    ...
    if (process.platform === "win32" && !isUsingTestDir) {
      ...
      const messageResult = await dialog.showMessageBox({
        ...
        detail:
          "LINEアプリは起動直後の最初の着信時に、強制的にデフォルトの着信音に戻す処理が行われます。\n\nそのためLINEアプリを起動中に本アプリケーションを用いて着信音を置換してください。",
        buttons: ["続行", "キャンセル"],
        defaultId: 0,
        cancelId: 1,
      });
    ...
    if (process.platform === "win32") {
      ...
      return new Promise((resolve) => {
        ...
        const encodedCommand = Buffer.from(finalPsCommand, "utf16le").toString(
          "base64"
        );
...
ipcMain.handle("check-backup-exists", async () => {
  try {
    ...
    if (
      backupExistsCache &&
      Date.now() - backupExistsCache.timestamp < CACHE_TTL
    ) {
      console.log(
        "バックアップ存在確認: キャッシュを使用",
        backupExistsCache.exists
      );
    ...
    if (process.env.NODE_ENV === "development") {
      ...
      if (process.platform === "win32") {
        ...
        return new Promise((resolve) => {
          ...
          const encodedCommand = Buffer.from(psCommand, "utf16le").toString(
            "base64"
          );
          const command = `powershell.exe -EncodedCommand ${encodedCommand}`;
  ...
  try {
    ...
    if (isDev && getUseTestDirInDevFlag()) {
      ...
      const testDir = path.join(
        os.tmpdir(),
        "line-ringtone-replace-test-output"
      );
    ...
    console.error(
      "LINEデータディレクトリが見つかりません。手動でインストールされているか確認してください。"
    );
    ...
    if (
      ...
      console.log(
        "開発環境: ディレクトリが見つからないためフォールバックパスを使用します:",
        fallbackDir
      );
      return fallbackDir;
    }
...
async function replaceRingtone(filePath: string) {
  try {
    ...
    if (!isAudio) {
      return {
        success: false,
        message: "選択されたファイルは音声ファイルではありません。",
      };
    }
    ...
    if (!fs.existsSync(ringtoneDir)) {
      try {
        ...
        console.log(
          "通常権限でのディレクトリ作成に失敗しました、管理者権限で試行します:",
          e
        );
    ...
    if (process.platform === "win32") {
      ...
      return new Promise((resolve) => {
        ...
        // ディレクトリ作成
        psCommand.push(
          `if (-not (Test-Path "${ringtoneDir}")) { New-Item -Path "${ringtoneDir}" -ItemType Directory -Force }`
        );

        // バックアップ作成（必要な場合）
        if (fs.existsSync(ringtonePath) && !hasBackup) {
          psCommand.push(
            `if (-not (Test-Path "${backupPath}")) { Copy-Item "${ringtonePath}" "${backupPath}" -Force }`
          );
        }
            ...
            if (error) {
              ...
              try {
                ...
                if (!fileExists) {
                  ...
                  // さらに詳細な情報を提供
                  let detailedMessage =
                    "着信音ファイルの置換に失敗しました。以下の原因が考えられます：\n";
                  detailedMessage +=
                    "・LINEアプリが実行中でファイルがロックされている\n";
                  detailedMessage += "・管理者権限が正しく機能していない\n";
                  detailedMessage += "・ファイルパスに問題がある\n\n";
                  detailedMessage +=
                    "LINEアプリを完全に終了してから再試行してください。";
              ...
              try {
                ...
                if (!fileExists) {
                  ...
                  resolve({
                    success: false,
                    message:
                      "着信音ファイルの置換に成功しましたが、ファイルが見つかりません。権限の問題が発生している可能性があります。",
                  });
                  return;
                }
...
function getUseTestDirInDevFlag(): boolean {
  ...
  return true; // 実際のLINEディレクトリを使用（必要に応じてtrueに変更可能）
}
